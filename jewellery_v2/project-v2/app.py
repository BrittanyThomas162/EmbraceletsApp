#Authors: DOMINIQUE REDDICKS, BRITTANY THOMAS​, JEVAUGHN MAYNE ​
#         KRISHNA JONES , BOSWELL CHANCE , DANEILLIA BRUKE
...
@app.route('/') #route describes path taken to webpage
@app.route('/index')
#function for first webpage
def index():
   return render_template('./logins.html') #render_template displays html as webpage
@app.route('/loginAsAdmin')
#function for admin webpage
def asAdmin():
   return render_template('./loginAsAdmin.html')
@app.route('/loginAsDeliveryGuy')
#function for deliverer webpage
def asDeliveryGuy():
...
#function to display inventory to admin
def administration():
   if session.get('username'):
      cursor = mysql.connection.cursor()
      cursor.execute('''SELECT * FROM inventory ''')
...
@app.route('/delivery')
#function to display delivery list
def delivery():
   if session.get('username'):
      cursor = mysql.connection.cursor()
      st = "pending"
...
def signup_page():
   if request.method == 'GET':
      if session.get('username'):
         return redirect(url_for('home_page')) #redirects user to another webpage
      else:
...
#function for admin sign-in page
def admin_signin():
   if request.method == 'GET':
      if session.get('username'):
...
#function for deliverer sign-in page
...
#function to display and edit profile information on webpage
def edit_profile():
   if request.method == 'GET':
      if session.get('username'):
         cursor = mysql.connection.cursor()
...
#function to display error encountered during the purchasing process
...
@app.route('/logout')
#logout function to exit a particular account/profile
def logout_page():
   session['username'] = None
   return redirect(url_for('home_page'))

@app.route('/confirm_purchase', methods=['POST'])
#function to confirm any purchase being made
def confirm_page():
   if request.method == 'POST':
      if session.get('username'):
         date = datetime.now()
...
#function that processes info recieved for an item and updates database accordingly
def add_item():
   if request.method == 'POST':
      if session.get('username'):
...
#function to display customer purchase history
def history_page(id):
   if (session.get('username')):
...
#functiton to delete an order made by the customer
def deleting_order():
   if session.get('username'):
      try:
...
#function to delete an item from the order being made
def deleting_item():
   if session.get('username'):
      try:
...
#function to edit details of order
def editing_order():
      if session.get('username'):
...
#function to add or subtract from the quantity of an item in the database
...
#function to indicate successful order delivery
def successful():
   if request.method == "POST":
      id = request.form['id']
      cursor = mysql.connection.cursor()
...
#function to indicate failed order delivery
def failed():
   if request.method == "POST":
      id = request.form['id']
      cursor = mysql.connection.cursor()
